...
class AccountInvoice(models.Model):
    ...
    def _convert_to_local_currency(self, amount):
        ...
        #No vamos a usarlo, ya que el calculo se hara con la tasa del reporte
        #
        #
        ...
        #if self.currency_id != moneda_dop:
        #    currency_id = self.currency_id.with_context(
        #        date=self.invoice_date)
        #    round_curr = currency_id.round
        #    amount = round_curr(currency_id.compute(amount, moneda_dop))
    ...
    @api.depends('line_ids.price_subtotal', 'line_ids.tax_base_amount','line_ids.tax_line_id', 'partner_id', 'currency_id', 'state')
        ...
        for inv in self:
            if inv.type == 'in_invoice' and inv.state != 'draft':
                ...
                for tax in inv._get_tax_line_ids():
                    if tax.tax_line_id.purchase_tax_type == 'isr':
                        isr = tax.tax_line_id.isr_retention_type
    ...
    @api.depends('line_ids.tax_base_amount','line_ids.tax_line_id', 'state')
    ...
    def _compute_is_exterior(self):
        for inv in self:
            tipo = str(inv.l10n_latam_document_number)
            tipo = tipo[:3]
            #tipo=''
            inv.is_exterior = True if (tipo == 'B17' or tipo == 'E47') else False
    ...
    def ext_onchange_journal_id(self):
        self.l10n_do_expense_type = False
        self.service_type_detail = False
