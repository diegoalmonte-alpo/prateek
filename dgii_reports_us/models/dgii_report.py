...
class DgiiReport(models.Model):
    ...
    tasa = fields.Float(string='Tasa Oficial',
                        digits=0,
                        help='Tasa Promedio del Banco Central.')
    ...
    def _get_country_number(self, partner_id):
        ...
        country code
        Revisar mas profundo a ver que sucede
        """
        res = False
    ...
    def _compute_606_data(self):
        for rec in self:
            ...
            tasa = 1
            report_data = ''
            moneda_dop = self.env['res.currency'].search([('name', '=', 'DOP')])
            ...
            for inv in invoice_ids:
                ...
                rnc_ced = self.formated_rnc_cedula(
                    inv.partner_id.vat
                ) if inv.l10n_latam_document_type_id.l10n_do_ncf_type != 'exterior' else \
                    self.formated_rnc_cedula(
                    inv.company_id.vat)
                show_payment_date = self._include_in_current_report(inv)

                #Para pasar a la tasa todas las facturas hechas en US$
                if inv.currency_id != moneda_dop:
                    tasa = self.tasa
                else:
                    tasa = 1
                ...
                values = {
                    ...
                    'service_total_amount': (inv.service_total_amount * tasa),
                    'good_total_amount': (inv.good_total_amount * tasa),
                    'invoiced_amount': (inv.amount_untaxed * tasa),
                    'invoiced_itbis': (inv.invoiced_itbis * tasa),
                    'proportionality_tax': (inv.proportionality_tax * tasa),
                    'cost_itbis': (inv.cost_itbis * tasa),
                    'advance_itbis': (inv.advance_itbis * tasa),
                    ...
                    show_payment_date else 0,
                    'selective_tax': (inv.selective_tax * tasa),
                    'other_taxes': (inv.other_taxes * tasa),
                    'legal_tip': (inv.legal_tip * tasa),
    ...
    def _process_op_dict(self, args, invoice):
        op_dict = args
        if invoice.l10n_latam_document_type_id.l10n_do_ncf_type and invoice.type != 'out_refund':
            ...
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    'withholding_date': inv.payment_date if (
                        ...
                        show_payment_date) else False,
                    'invoiced_amount': inv.amount_untaxed_signed * tasa,
                    #'invoiced_amount': (inv.amount_factura_original - inv.invoiced_itbis), #solo para Dealers
                    'invoiced_itbis': inv.invoiced_itbis * tasa,
                    'third_withheld_itbis': inv.third_withheld_itbis * tasa
    ...
    def generate_report(self):
        if self.state == 'generated':
            action = self.env.ref(
                'dgii_reports_us.dgii_report_regenerate_wizard_action').read()[0]
    ...
    def get_606_tree_view(self):
        return {
            ...
            'view_id':
                self.env.ref('dgii_reports_us.dgii_report_purchase_line_tree').id,
    ...
    def get_607_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('dgii_reports_us.dgii_report_sale_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)]
        }
    ...
    def get_608_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('dgii_reports_us.dgii_cancel_report_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)]
        }
    ...
    def get_609_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('dgii_reports_us.dgii_exterior_report_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)]
        }
