...
class print_vendor_statement(models.AbstractModel): 
    _name = 'report.dev_vendor_account_statement.vendor_statement_template'
    ...
    def _lines_get(self, partner):
        ...
        movelines = moveline_obj.search([('partner_id', '=', partner.id),
                                         ('currency_id', '=', partner.report_currency_id.id),
        ...
        if move_lines:
            for line in move_lines:
                ...
                if line.type == 'in_invoice':
                    inv_amt = line.amount_total
                    inv_residual = line.amount_residual
                    paid_amt = inv_amt - inv_residual
                ...
                if total > 0 or total < 0:
                    res.append({
                                'date':line.date,
                                'desc':line.ref or '/',
                                'ref':line.numero_factura or '',
                                'currency_id':line.currency_id,
                                'date_maturity':line.invoice_date_due, #_maturity
                                'debit':float(inv_amt),
