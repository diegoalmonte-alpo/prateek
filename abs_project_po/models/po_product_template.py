...
class PurchaseOrderProductTemplate(models.Model):
    ...
    def display_pending_purchase_order_qty(self):        
        if self.id:
            template_id = self.env.ref('abs_project_po.purchase_order_line_tree_custom_view').id
            product_ids=[]
            ...
            return {
                    ...
                    'view_id': template_id,
                    'views': [(self.env.ref('abs_project_po.purchase_order_line_tree_custom_view').id, 'tree')],
